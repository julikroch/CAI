    ...
    class Program
    {
        ...
        public static string verificarRegistro()
        {
            ...
            do
            {
                ...
                { // Recorro el array para encontrar al alumno ingresado y valido que su condicion no sea 'Libre'
                    alumnoEncontrado = true;
                    do
                    {
                        string pInscripcion = srInscripcion.ReadLine();
                        string[] arrInscripcion = pInscripcion.Split(';');
                        if (registro == Convert.ToInt32(arrInscripcion[0]))
                        { // Recorro el array de inscripciones para validar que no haya una inscripicion previa
